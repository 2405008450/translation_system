...
| `ARQ_MAINTENANCE_MAX_JOBS` | 2 | 普通维护队列并发，主要处理手动 QA 和旧版遗留任务 |
| `ARQ_AUTO_TM_MAX_JOBS` | 1 | auto-TM outbox 与自动重匹配队列并发，建议保持 1，避免多文件重匹配同时压数据库 |
...
| `app_import_tasks` | `/app/data/import_tasks` | 上传与资源导入暂存（app 与各 worker **必须共享**） |
...
- 确认各 worker 已挂载 `app_import_tasks` 卷（`docker-compose.prod.yml` 已配置）
- 重建 app 与各 worker：`scripts/deploy_prod.sh restart`
