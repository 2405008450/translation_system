...
def get_tm_candidates(
    ...
    exact_stmt = _apply_tm_scope_filter(
        exact_stmt,
    ...
    if not _allow_fuzzy_text(match_text):
        candidates.sort(key=lambda c: c.score, reverse=True)
        return candidates[:max_candidates]

    # 模糊匹配
    trigram_threshold = _get_trigram_prefilter_threshold(similarity_threshold)
    min_length, max_length = _build_fuzzy_length_bounds(match_text)
    params = {
...
def _allow_fuzzy_lookup(sentence: PreparedSentence) -> bool:
...
def _build_fuzzy_length_bounds(match_text: str) -> tuple[int, int]:
    normalized = normalize_match_text(match_text) or (match_text or "").strip()
    length = len(normalized)
    if length <= 0:
        return 1, 1
    min_length = max(1, int(length * FUZZY_LENGTH_MIN_RATIO) - FUZZY_LENGTH_MIN_MARGIN)
    max_length = max(length + FUZZY_LENGTH_MAX_MARGIN, int(length * FUZZY_LENGTH_MAX_RATIO) + FUZZY_LENGTH_MAX_MARGIN)
...
def _resolve_matches(
    ...
    if not include_fuzzy:
        for index, _ in unresolved_items:
            resolved_matches[index] = ResolvedMatch(
                status="none",
                score=0.0,
                matched_source_text=None,
...
def _find_fuzzy_matches(
    ...
    matches: list[ResolvedMatch | None] = [None] * len(prepared_sentences)
    lookup_items = [
        (index, sentence)
        for index, sentence in enumerate(prepared_sentences)
        if _allow_fuzzy_lookup(sentence)
    ]
    if not lookup_items:
        return matches, 0
    ...
    total_candidates = 0
    offset = 0
    # 仅合并完整输入相同的查询，保留原文与辅助文本差异，避免复用错误的差异标记。
    indexes_by_sentence: dict[PreparedSentence, list[int]] = {}
    for index, sentence in lookup_items:
        indexes_by_sentence.setdefault(sentence, []).append(index)
    lookup_sentences = list(indexes_by_sentence)
    snapshot_context = batch_search_snapshot(
        db, query_count=len(lookup_sentences) if _should_use_tm_search_projection(db) else 0,
        collection_ids=collection_ids, source_language=source_language, target_language=target_language,
    )
    with snapshot_context as search_table:
        for chunk in _chunked(lookup_sentences, _get_fuzzy_match_batch_size()):
            ...
            for local_index, match in enumerate(chunk_matches):
                sentence = lookup_sentences[offset + local_index]
                for duplicate_index, original_index in enumerate(indexes_by_sentence[sentence]):
                    matches[original_index] = deepcopy(match) if duplicate_index else match
            offset += len(chunk)
        ...
        return text(
            ...
            LEFT JOIN LATERAL (
                ...
                FROM (
                    ...
                    FROM {search_table or 'memory_entry_search'} AS ts
                    WHERE ts.source_normalized % input.query_text
                      AND ts.source_length BETWEEN input.min_length AND input.max_length
                      {"AND int4range(ts.source_length, ts.source_length, '[]') <@ int4range(input.min_length, input.max_length, '[]')" if search_table else ""}
                      {collection_filter_sql}
                      {language_filter_sql}
                    ORDER BY {('ts.source_normalized <-> input.query_text, ' if search_table else '')}similarity(ts.source_normalized, input.query_text) DESC, ts.updated_at DESC, ts.entry_id ASC
                    LIMIT :candidate_limit
                ) AS ts
                JOIN memory_entries AS tm ON tm.id = ts.entry_id
                ...
                ORDER BY ts.trigram_score DESC, ts.updated_at DESC, ts.entry_id ASC
            ) AS matched ON TRUE
    ...
    return text(
        ...
        LEFT JOIN LATERAL (
            ...
            WHERE tm.source_normalized IS NOT NULL
              ...
              AND char_length(tm.source_normalized) BETWEEN input.min_length AND input.max_length
              {collection_filter_sql}
              {language_filter_sql}
            ORDER BY similarity(tm.source_normalized, input.query_text) DESC, tm.updated_at DESC, tm.id ASC
            LIMIT :candidate_limit
        ) AS matched ON TRUE
        ORDER BY input.query_index ASC
        """
...
@contextmanager
def document_statistics_match_timeout(db: Session):
    """仅在当前统计会话中放宽 TM 超时，结束后恢复事务原配置。"""
    key = "document_statistics_tm_timeout_ms"
    previous_override = db.info.get(key)
    previous_timeout = db.execute(text("SHOW statement_timeout")).scalar_one()
    db.info[key] = get_settings().document_statistics_tm_timeout_ms
    try:
        yield
        # SQL 失败时事务已中止，由调用方回滚，不能再发恢复语句掩盖原异常。
        db.execute(
            text("SELECT set_config('statement_timeout', :timeout_value, true)"),
            {"timeout_value": previous_timeout},
        )
    finally:
        if previous_override is None:
            db.info.pop(key, None)
        else:
            db.info[key] = previous_override


def _apply_match_statement_timeout(db: Session) -> None:
    timeout_ms = getattr(db, "info", {}).get("document_statistics_tm_timeout_ms")
...
def _normalize_language_pair(
    ...
    normalized_source = (source_language or "").strip() or None
    normalized_target = (target_language or "").strip() or None
    return normalized_source, normalized_target


def _apply_tm_scope_filter(
    stmt,
    collection_ids: list[UUID] | None,
    source_language: str | None,
    target_language: str | None,
    ...
    if not collection_ids:
        ...
        scoped_stmt = stmt.where(MemoryEntry.collection_id.in_(collection_ids))
    if source_language:
        scoped_stmt = scoped_stmt.where(MemoryEntry.source_language == source_language)
    if target_language:
        scoped_stmt = scoped_stmt.where(MemoryEntry.target_language == target_language)
    return scoped_stmt
