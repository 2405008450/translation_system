...
class Settings(BaseSettings):
    ...
    tm_batch_snapshot_enabled: bool = True
    tm_batch_snapshot_min_queries: int = Field(default=100, ge=1)
    tm_batch_snapshot_max_entries: int = Field(default=1000000, ge=1)
    # 文档统计需要完整扫描句段，独立于编辑器交互查询的超时限制。
    document_statistics_tm_timeout_ms: int = Field(default=120000, ge=1)
