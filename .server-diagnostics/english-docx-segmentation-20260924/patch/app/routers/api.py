...
from app.services.matcher import (
    ...
    get_tm_candidates_for_text,
    match_sentences_with_stats,
)
...
def _process_file_record_import(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    document_parse_options = new_import_options(document_parse_options, filename, resolved_source_language)
...
def _process_project_source_import(db: Session, task_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for file_payload in expanded_payloads:
        ...
        source_path = get_import_file_path(file_payload) if is_path_based_ai else None
        raw_bytes = None if is_path_based_ai else read_import_file_bytes(file_payload)
        file_parse_options = new_import_options(document_parse_options, filename, resolved_source_language)
        for resolved_target_language in resolved_target_languages:
            ...
            workspace_data = build_task_workspace(
                ...
                target_language=resolved_target_language,
                document_parse_mode=document_parse_mode,
                document_parse_options=file_parse_options,
            )
            create_record = (
                create_file_record_with_segments_from_path
                if source_path is not None
...
def compute_project_document_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=504,
            detail="文档统计的记忆库匹配超时，未生成不完整报告。请减少本次选择的文件后重试。",
        ) from exc
    for file_record in files:
        ...
        if source_path is not None and Path(source_filename).suffix.lower() in SUPPORTED_EXTENSIONS:
