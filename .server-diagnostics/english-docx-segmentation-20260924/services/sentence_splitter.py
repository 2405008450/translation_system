...
def split_sentence_spans(text: str, *, preserve_dotted_names: bool = False) -> list[SentenceSpan]:
    from app.services.docx_segmentation import PROFILE
    if PROFILE.get() == 'en_docx_v1':
        return _split_english_docx_spans(text)
...
def _split_english_docx_spans(text: str) -> list[SentenceSpan]:
    """英文规则只在版本化 DOCX 上下文启用；所有范围指向原始字符。"""
    scan = _prepare_text_for_span_scan(text)
    protected = set()
    for match in re.finditer(r'(?:https?://|www\.)[^\s<>]+|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}', scan):
        end = match.end()
        while end > match.start() and scan[end-1] in '.!?\"\')]}':
            end -= 1
    ...
    while i < len(scan):
        ...
        if char in '\r\n':
            line_end = i+1
            if char == '\r' and scan[line_end:line_end+1] == '\n':
                line_end += 1
            previous = scan[start:i].strip()
            following = scan[line_end:].split('\n', 1)[0].strip()
            boundary = bool(previous) and (
                scan[line_end:line_end+1] in {'\r', '\n'}
                or looks_like_numbered_heading(previous)
                or looks_like_numbered_heading(following)
                or looks_like_short_heading_before_body(previous, following)
                or bool(re.match(r'[•●▪◦]\s', following))
            )
        elif char in '.!?' and i not in protected:
            boundary = True
            if char == '.':
                before = scan[:i]
                token = re.search(r'([A-Za-z0-9.]+)$', before)
                word = token.group(1) if token else ''
                tail = scan[i+1:]
                following = tail.lstrip('\"\'’”)]} \t\r\n')
                next_word = re.match(r'[A-Za-z]+', following)
                lower = word.lower()
                if i+1 < len(scan) and scan[i+1].isascii() and scan[i+1].isalnum():
                    boundary = False
                elif following:
                    if lower in {'mr', 'mrs', 'ms', 'dr', 'prof', 'sr', 'jr'}:
                        boundary = False
                    elif lower in {'fig', 'figs', 'no', 'nos', 'vol', 'pp', 'p', 'sec', 'art'} and following[0].isdigit():
                        boundary = False
                    elif re.fullmatch(r'[A-Z]', word) and re.match(r'[A-Z][a-z]+|[A-Z]\.', following):
                        boundary = False
                    elif lower in {'e.g', 'i.e'}:
                        boundary = False
                    elif lower in COMMON_ABBREVIATIONS or INITIALISM_PATTERN.fullmatch(word):
                        boundary = bool(next_word and next_word.group(0)[0].isupper() and next_word.group(0).lower() in COMMON_SENTENCE_STARTERS)
                    elif re.fullmatch(r'\d+|[ivxlcdmIVXLCDM]+|[A-Z]', word) and not scan[start:i-len(word)].strip():
                        boundary = False
        if boundary:
            end = i if char in '\r\n' else i+1
            if char not in '\r\n':
                ...
                while end < len(scan) and scan[end] in TRAILING_SENTENCE_CLOSERS:
                    end += 1
            lo, hi = start, end
            while lo < hi and scan[lo].isspace():
                lo += 1
            while hi > lo and scan[hi-1].isspace():
                hi -= 1
            if hi > lo:
                spans.append(SentenceSpan(lo, hi))
            start = end
            i = max(i+1, end)
        else:
            i += 1
    lo, hi = start, len(scan)
    while lo < hi and scan[lo].isspace():
        lo += 1
    while hi > lo and scan[hi-1].isspace():
        hi -= 1
