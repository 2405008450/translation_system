...
from app.services.docx_segmentation import segmentation_scope, prepare_package
...
def _group_segments_by_block(
    ...
    source_by_origin = {
        _docx_origin_key(item): item for item in source_segment_list if _docx_origin_key(item)
    }
    ...
    for segment in segment_list:
        ...
        # 新版英文句段优先按持久化的原始 OOXML 范围定位，不依赖重排后的 ID。
        origin_source = source_by_origin.get(_docx_origin_key(segment))
        if origin_source is not None:
            block_key = _source_segment_block_key(origin_source)
...
def _docx_origin_key(segment: Any) -> tuple | None:
    metadata = _get_segment_value(segment, "segment_metadata") or {}
    ...
    return tuple((s.get("part"), s.get("paragraph_index"), s.get("start"), s.get("end")) for s in sources) or None
