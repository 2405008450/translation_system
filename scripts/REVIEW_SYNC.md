...
1. 首次安装执行 `scripts/migrations/0022_add_review_sync.sql`；已安装旧版功能的项目执行 `scripts/migrations/0029_enable_review_sync_by_default.sql`。新项目默认开启，升级会一次性开启现有项目（旧版本未区分默认关闭和手动关闭）。后续手动关闭的项目不会被重复迁移或重启重新开启。历史修订不关联，运行时结构补齐也包含相同迁移。
2. 部署后端及现有 segment-sync worker，再部署前端。失焦请求唤醒现有 worker；API 进程每 15 秒补偿扫描持久化修订任务，进程重启后可继续消费。
3. 项目设置的“自动应用与锁定”中，“修订同步”默认开启，可按需关闭。原有片段级同步禁用仍有效，旧的失焦白名单不限制修订同步。
