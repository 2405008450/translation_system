...
const entries = ref<TermEntryRecord[]>([])
...
const entryColumns: DataTableColumn[] = [
  ...
  { key: 'target_text', label: '术语译文' },
  { key: 'updated_at', label: '更新时间', width: '160px' },
]

const entryCountLabel = computed(() => `${termBase.value?.entry_count ?? totalEntries.value} 条术语`)
...
function resetBaseForm(item: TermBase) {
...
function startEditing(entry: TermEntryRecord | Record<string, any>) {
...
async function loadBase() {
  ...
  try {
    const { data } = await http.get<TermBase>(`/term-bases/${props.id}`)
    termBase.value = data
    resetBaseForm(data)
    pageError.value = ''
  } catch (error) {
    termBase.value = null
    pageError.value = getErrorMessage(error, '术语库详情加载失败。')
  } finally {
...
async function loadEntries() {
  ...
  try {
    const { data } = await http.get<PaginatedResponse<TermEntryRecord>>(
      `/term-bases/${props.id}/entries`,
      {
        params: {
    ...
    entryMessage.value = getErrorMessage(error, '术语条目加载失败。')
  } finally {
...
async function saveBase() {
  ...
  try {
    ...
    const { data } = await http.put<TermBase>(`/term-bases/${termBase.value.id}`, payload)
    termBase.value = data
    resetBaseForm(data)
    baseMessage.value = '术语库信息已更新。'
    await loadEntries()
  } catch (error) {
    baseMessage.value = getErrorMessage(error, '术语库保存失败。')
  } finally {
...
async function saveEntry() {
  ...
  try {
    const { data } = await http.put<TermEntryRecord>(`/term-entries/${editingEntryId.value}`, {
      source_text: editingSourceText.value,
      target_text: editingTargetText.value,
    })
    entryMessage.value = '术语条目已更新。'
    await loadEntries()
    ...
    entryMessage.value = getErrorMessage(error, '术语条目保存失败。')
  } finally {
...
<template>
  <div class="content-stack">
    ...
    <section v-if="loadingBase && !termBase" class="panel">
    ...
    <template v-else-if="termBase">
      <section class="panel">
        <div class="panel-header panel-header--compact">
          <div>
            <div class="section-title section-title--tight">基础信息</div>
            <p class="panel-subtitle">修改名称、说明和语言对后，术语条目的语言标签会一起同步。</p>
          </div>
          <span class="tag">最近更新：{{ formatDate(termBase.updated_at) }}</span>
        </div>

        <div class="upload-form form-grid-2" style="margin-top: 0;">
          <label class="field">
            <span class="field__label">术语库名称</span>
            <input v-model="formName" class="field__control" type="text" placeholder="请输入术语库名称" />
          </label>
          ...
          <div class="field-actions">
            <button class="button button--primary" type="button" :disabled="savingBase" @click="saveBase">
              <Loader2 v-if="savingBase" class="lucide-spin" />
              <Save v-else :size="14" />
              {{ savingBase ? '保存中...' : '保存基础信息' }}
            </button>
          </div>
        </div>

        <p v-if="baseMessage" class="form-message" :class="{ 'is-error': baseMessage.includes('失败') || baseMessage.includes('不能为空') }">
          {{ baseMessage }}
        </p>
      </section>
      ...
      <section class="panel panel--stretch">
        ...
        <div v-if="editingEntry" class="upload-panel" style="margin-top: 6px;">
          <div class="section-title" style="margin-bottom: 10px;">编辑中的术语</div>
          <div class="upload-form form-grid-2" style="margin-top: 0;">
            <label class="field">
