...
| `ARQ_IMPORT_MAX_JOBS` / `ARQ_MAINTENANCE_MAX_JOBS` / `ARQ_PRETRANSLATION_MAX_JOBS` / `PRETRANSLATION_RUN_FILE_CONCURRENCY` | 上传与资源导入队列、后台维护队列、预翻译 run 队列和单个预翻译批次内文件并发控制 |
